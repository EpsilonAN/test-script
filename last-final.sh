...
TARGET_FILES_ZIP=$(find out/target/product -name "*-target_files.zip" -type f | head -n 1)

if [ -z "$TARGET_FILES_ZIP" ]; then
    echo "Error: target_files.zip not found! Build might have failed."
    exit 1
fi

echo "Signing target files: $TARGET_FILES_ZIP"
